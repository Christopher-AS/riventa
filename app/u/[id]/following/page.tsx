...
type Item = {
  id: string;
  createdAt: string;
  following: { id: string; email: string };
};
...
export default async function Page({
  ...
  const res = await fetch(`${base}/api/following?${qs.toString()}`, {
    cache: "no-store",
  });
    ...
    return (
      <main className="max-w-2xl mx-auto p-6">
        ...
        <p className="mt-4 text-red-600">
          Erro ao carregar seguindo: {res.status} {res.statusText}
        </p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-2xl mx-auto p-6 space-y-4">
      ...
      <ul className="divide-y divide-gray-200 rounded-2xl border border-gray-200 bg-white">
        {data.items.length === 0 ? (
          <li className="p-4 text-gray-600">Não segue ninguém.</li>
        ) : (
          data.items.map((it) => (
            <li key={it.id} className="p-4 flex items-center justify-between">
              <div className="flex flex-col">
                <Link
                  href={`/u/${it.following.id}`}
      ...
      <div className="flex justify-between">
        ...
        {data.nextCursor ? (
          <Link
            href={`/u/${params.id}/following?limit=${limit}&cursor=${data.nextCursor}`}
