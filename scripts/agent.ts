...
const playbook = fs.existsSync('AGENTS.md') ? fs.readFileSync('AGENTS.md', 'utf8') : '';
...
async function toolRouter(name: string, argsStr: string) {
  ...
  if (name === 'exec') {
    ...
    try {
      const { stdout, stderr } = await exec(cmd, { cwd, maxBuffer: 1024 * 1024 * 16 });
...
async function run(goal: string) {
  ...
  for (let step = 0; step < 24; step++) {
    ...
    const texts        = outputs.filter(o => o.type === 'message' || o.type === 'output_text');

    // imprime qualquer texto/explicação
    const outText =
      res.output_text ||
      texts.map((t: any) => t.text).filter(Boolean).join('\n') ||
      '';
